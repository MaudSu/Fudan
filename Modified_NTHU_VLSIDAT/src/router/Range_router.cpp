...
//#define THREE_BEND_ROUTE

#define PSD_PRINT

vector<Range_element*>psd_ntugr_stage_forbidden_region;
...
{
#ifdef PSD_EDGE_ORDER

#else
	return congestionMap2d->edge(a->grid->x, a->grid->y, a->dir).congestion() >
           congestionMap2d->edge(b->grid->x, b->grid->y, b->dir).congestion();
#endif // PSD_EDGE_ORDER
}

static bool psd_ntugr_stage_comp_grid_edge(const Grid_edge_element* a, const Grid_edge_element* b)
{
    return congestionMap2d->edge(a->grid->x, a->grid->y, a->dir).congestion() >
           congestionMap2d->edge(b->grid->x, b->grid->y, b->dir).congestion();
}
...
static void psd_ntugr_stage_expand_range(int x1, int y1, int x2, int y2, int interval_index){
    Coordinate_2d start,end,cur_start,cur_end;
	int i,j,edge_num;
	double total_cong,avg_cong;
	...
	y2 = max(y1,y2);
    //obtain the expanded boundary. This is the very first expand, and the expand unit is 10.
    int PSD_EXPAND_RANGE_SIZE = 1;
	start.x = max(x1 - PSD_EXPAND_RANGE_SIZE, 0);
	end.x = min(x2 + PSD_EXPAND_RANGE_SIZE, rr_map->get_gridx() - 1);
	start.y = max(y1 - PSD_EXPAND_RANGE_SIZE, 0);
	end.y = min(y2 + PSD_EXPAND_RANGE_SIZE, rr_map->get_gridy() - 1);

    //right
    total_cong = 0;
	edge_num = 0;
	double psd_threshold = max(1.4 - ( cur_iter - routing_parameter->get_iteration_p2() ) * 1.0 / 100 , 1.2);
    for(i = start.y; i < end.y; i++){
        total_cong += congestionMap2d->edge(end.x, i, 0).cur_cap;
        edge_num += congestionMap2d->edge(end.x, i, 0).max_cap;
    }
    while(total_cong / edge_num > psd_threshold){
        if(end.x < rr_map->get_gridx()-1){
            end.x ++;
        }else{
            break;
        }
	...
	edge_num = 0;
    for(i = start.y; i < end.y; i++){
        total_cong += congestionMap2d->edge(start.x, i, 0).cur_cap;
        edge_num += congestionMap2d->edge(start.x, i, 0).max_cap;
    }
    while(total_cong / edge_num > psd_threshold){
        if(start.x > 0){
            start.x --;
	...
	edge_num = 0;
    for(i = start.x; i < end.x; i++){
        total_cong += congestionMap2d->edge(i, end.y, 3).cur_cap;
        edge_num += congestionMap2d->edge(i, end.y, 3).max_cap;
    }
    while(total_cong / edge_num > psd_threshold){
        if(end.y < rr_map->get_gridy()-1){
            end.y ++;
        }else{
            break;
        }
	...
	edge_num = 0;
    for(i = start.x; i < end.x; i++){
        total_cong += congestionMap2d->edge(i, start.y, 3).cur_cap;
        edge_num += congestionMap2d->edge(i, start.y, 3).max_cap;
    }
    while(total_cong / edge_num > psd_threshold){
        if(start.y > 0){
             start.y --;
    ...
    psd_ntugr_stage_start_x = start.x;
    psd_ntugr_stage_start_y = start.y;
    psd_ntugr_stage_end_x = end.x;
    psd_ntugr_stage_end_y = end.y;
    for (i = start.x; i <= end.x; ++i) {
		for (j = start.y; j <= end.y; ++j){
            expandMap->color(i, j) = interval_index;
		}
    }
    psd_ntugr_stage_forbidden_region.push_back(new Range_element(start.x, start.y, end.x, end.y));

    //right
    total_cong = 0;
	edge_num = 0;
	psd_threshold = max( 0.9 - ( cur_iter - routing_parameter->get_iteration_p2() ) * 1.0 /100, 0.7);
    for(i = start.y; i < end.y; i++){
        total_cong += congestionMap2d->edge(end.x, i, 0).cur_cap;
        edge_num += congestionMap2d->edge(end.x, i, 0).max_cap;
    }
...
//    if ( !check_path_no_overflow(&two_pin->path,two_pin->net_id, false) || (cur_iter > 100 && !check_path_no_overflow(&two_pin->path,two_pin->net_id, true)))
	{
		++total_twopin;
  ...
  #ifdef THREE_BEND_ROUTE

#endif // THREE_BEND_ROUTE
...
static void psd_initial_MM_range_router(Two_pin_element_2d * two_pin)
{
	if ( !check_path_no_overflow(&two_pin->path,two_pin->net_id, false) )
	{
		++total_twopin;
        ...
        bool find_path_flag = false;
             ...
             find_path_flag = monotonic_pattern_route(two_pin->pin1.x, two_pin->pin1.y,
                    two_pin->pin2.x, two_pin->pin2.y,
                    two_pin,two_pin->net_id,
                    bound_cost,
                    bound_distance,
...
static void psd_refinement_WL_range_router(Two_pin_element_2d * two_pin)
{
//	if ( check_path_no_overflow(&two_pin->path,two_pin->net_id, false) )
//	{
		++total_twopin;
...
static void psd_ntugr_stage_range_router(Two_pin_element_2d * two_pin)
{
	if ( !check_path_no_overflow(&two_pin->path,two_pin->net_id, false) )
  //  if ( !check_path_no_overflow(&two_pin->path,two_pin->net_id, true) )
	{
		++total_twopin;
        Coordinate_2d pin1 = two_pin->pin1;
        Coordinate_2d pin2 = two_pin->pin2;
                             ...
                             vector<Two_pin_element_2d *> *twopin_list)
{
	...
	int psdCenterX = (left_x + right_x) / 2;
	int psdCenterY = (bottom_y + top_y) / 2;
            ...
            if( (*it)->done != done_iter ) {
                   ...
                   routeStateMap->color((*it)->pin2.x, (*it)->pin2.y) != done_counter)
                {
                        ...
                        inside_range(left_x,bottom_y,right_x,top_y,&((*it)->pin2)) )
                    {
                        (*it)->done = done_iter;
                        //psd_24
                        (*it)->psd_dist = min(abs( (*it)->pin1.x - psdCenterX ) + abs( (*it)->pin1.y - psdCenterY ) , abs( (*it)->pin2.x - psdCenterX ) + abs( (*it)->pin2.y - psdCenterY ));
...
bool psd_comp_counter(const COUNTER &a, const COUNTER &b){
...
#ifdef PSD_INSERT_ALL_OF_NET

#endif // PSD_INSERT_ALL_OF_NET

#ifdef PSD_PRINT
        psd2pinNetNum += twopin_list.size();
#endif // PSD_PRINT
...
#ifdef PSD_PRINT
	printf("specify-net-num = %d    rip-up-net-num = %d\n",psd2pinNetNum,total_twopin);
#endif // PSD_PRINT
}

void psd_ntugr_stage_specify_all_range(void){
    vector<Two_pin_element_2d *> twopin_list;
	...
	for (int i = 0; i <= intervalCount - 1; ++i) {
		...
		range_vector.clear();
		//psd_24
		for(int s = psd_ntugr_stage_forbidden_region.size() -1; s >= 0; --s){
            delete psd_ntugr_stage_forbidden_region[s];
		}
		psd_ntugr_stage_forbidden_region.clear();

		sort(interval_list[i].grid_edge_vector.begin(),
             interval_list[i].grid_edge_vector.end(),
             psd_ntugr_stage_comp_grid_edge);
		...
		for (int j = 0; j<(int)interval_list[i].grid_edge_vector.size(); ++j)
		{
                 ...
                 (expandMap->color(nei_x, nei_y) != i) )
            {
                ...
                psd_ntugr_stage_expand_range(x, y, nei_x, nei_y, i);
            }
		}
        for (int j = 0; j<(int)range_vector.size(); ++j){
                    twopin_list.clear();
                    query_range_2pin(range_vector[j]->x1,
                             ...
                             range_vector[j]->y2,
                             &twopin_list);
                    sort(twopin_list.begin(),twopin_list.end(),psd_ntugr_stage_comp_stn_2pin);
                    for (int jjj = 0; jjj<(int)twopin_list.size(); ++jjj) {
                        psd_ntugr_stage_range_router(twopin_list[jjj]);
                    }
        }
	}
    ...
    psd_ntugr_stage_start_x = 0;
    psd_ntugr_stage_start_y = 0;
    psd_ntugr_stage_end_x = 0;
    psd_ntugr_stage_end_y = 0;
	twopin_list.clear();
	int length = two_pin_list.size();
	for (int i=0; i<length; ++i)
	{
	...
	{
        if(twopin_list[i]->boxSize() == 1) break;
		psd_ntugr_stage_range_router(twopin_list[i]);
	}

	mazeroute_in_range->clear_net_tree();
}

void psd_specify_all_range(void){
    total_twopin = 0;
    vector<Two_pin_element_2d *> temp_list;
    ...
    for(int i = two_pin_list.size() -1; i >= 0; --i)
    {
        ...
        for(int j = two_pin_list[i]->path.size()-1; j > 0; --j)
        {
            ...
            if (congestionMap2d->edge(two_pin_list[i]->path[j]->x, two_pin_list[i]->path[j]->y, edge_idx).isOverflow())
            {
                temp_list.push_back(two_pin_list[i]);
                break;
            }
    ...
    sort(temp_list.begin(),temp_list.end(),psd_initial_MM_comp_stn_2pin);
  //  printf("%d------p\n",temp_list.size());
    for(int i = 0; i < temp_list.size(); i++){
        psd_initial_MM_range_router(temp_list[i]);
    }
	...
	{
        if(temp_list[i]->boxSize() == 1) break;
		psd_initial_MM_range_router(temp_list[i]);
    }

    mazeroute_in_range->clear_net_tree();
#ifdef PSD_PRINT
	printf("rip-up-net-num = %d\n",total_twopin);
#endif // PSD_PRINT
}
void psd_refinement_WL_specify_all_range(void){
    total_twopin = 0;
    vector<Two_pin_element_2d *> temp_list;
    ...
    sort(temp_list.begin(),temp_list.end(),psd_initial_MM_comp_stn_2pin);
    for(int i = 0; i < temp_list.size(); i++){
        psd_refinement_WL_range_router(two_pin_list[i]);
    }
